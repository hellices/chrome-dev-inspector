...
export function getHtmlElementInfo(element) {
  ...
  while (current && current.parentElement && depth < maxDepth) {
    ...
    const parentClasses = current.classList.length > 0 ? `.${Array.from(current.classList).join('.')}` : '';
...
function formatCSSSection(css) {
  ...
  if (css.styles && Object.keys(css.styles).length > 0) {
    const styleKeys = Object.keys(css.styles).filter(k => css.styles[k] && css.styles[k] !== 'none' && css.styles[k] !== 'auto');
